...
function getDataFromMd(file, mdAttributes) {
    for (let key of Object.keys(mdAttributes)) {
      let filePath = path.dirname(file) + '/' + mdAttributes[key];

      if (fs.existsSync(filePath) &&
          /\.json/.test(path.extname(mdAttributes[key]))) {
          mdAttributes[key] = fs.readJsonSync(filePath)
      }
    }
...
module.exports = (file, data) => {
  ...
  return fs
    ...
    .then(fm => {
      return {
        data: merge({}, data, { page: getDataFromMd(file, fm.attributes) }),
